...
const hardcodedCurrent = [
  { role: "Managing Director, Ghana", org: "Shared Value Africa (SVAI)", period: "2022 — Present", location: "Accra, Ghana", description: "Leading SVAI's operations in Ghana, driving the shared value agenda across the private sector, government, and civil society. Building ecosystems where business success and community prosperity are mutually reinforced.", tags: ["Shared Value", "Ecosystem Building", "Private Sector"] },
  { role: "Head of New Business", org: "Shared Value Africa (SVAI)", period: "2021 — Present", location: "Pan-African", description: "Spearheading business development and partnership linkages across the African continent. Identifying and securing new opportunities that advance the shared value mission.", tags: ["Business Development", "Partnerships", "Fundraising"] },
  { role: "Ghana Focal Point", org: "JamiiTrade Africa", period: "2023 — Present", location: "Accra, Ghana", description: "Championing intra-African trade by connecting Ghanaian SMEs to the JamiiTrade peer-to-peer trading platform. Reducing barriers and expanding market access across the continent.", tags: ["Intra-African Trade", "SME Development", "AfCFTA"] },
  { role: "Conference Host and Panel Moderator", org: "Independent", period: "2018 — Present", location: "Pan-African", description: "An engaging conference host and panel moderator facilitating critical conversations at summits across Africa and beyond. Known for a warm, authoritative presence that turns panels into genuine conversations.", tags: ["Conference Hosting", "Panel Moderation", "Public Speaking"] },
];

const hardcodedPrevious = [
  { role: "Programme Officer", org: "Ghana Climate Innovation Centre (GCIC)", period: "2019 — 2021", location: "Accra, Ghana", description: "Supported climate-focused entrepreneurs with business development, fundraising, and programme management. Conducted tech startup maturity assessments connecting promising ventures to international programmes.", tags: ["Climate Innovation", "Startups", "Programme Management"] },
  { role: "Innovation Lab Facilitator", org: "UNLEASH Global", period: "2018 — 2020", location: "Multiple Countries", description: "Facilitated innovation labs bringing together 1,000 young leaders to develop solutions to the UN Sustainable Development Goals. Guided teams from personal insights to scalable solutions.", tags: ["SDGs", "Innovation", "Youth Leadership"] },
  { role: "Business Development Officer", org: "Aspen Network of Development Entrepreneurs (ANDE)", period: "2017 — 2019", location: "Accra, Ghana", description: "Supported small and growing businesses across Ghana through capacity building, investor connections, and ecosystem development initiatives.", tags: ["SME Support", "Investor Relations", "Capacity Building"] },
  { role: "Ecosystem Advisor", org: "African Youth, Gender and Climate Conference", period: "2023 — Present", location: "Pan-African", description: "Providing advisory support on event planning, speaker selection, and conference content creation for this annual pan-African conference.", tags: ["Advisory", "Youth", "Climate"] },
  { role: "Research and Policy Analyst", org: "Various Organisations", period: "2012 — 2016", location: "Ghana", description: "Conducted research and policy analysis across development, trade, and private sector engagement topics.", tags: ["Research", "Policy", "Analysis"] },
  { role: "Community Development Officer", org: "Social Enterprise Ghana", period: "2011 — 2013", location: "Accra, Ghana", description: "Worked at the grassroots level to support community-based enterprises, building the human foundations of ecosystem thinking from the ground up.", tags: ["Community Development", "Social Enterprise", "Grassroots"] },
];
...
export default function ExperienceClient({ experience: sanityExperience }: { experience: any[] }) {
  const sanityCurrent = sanityExperience.filter((e: any) => e.type === "Current");
  const sanityPrevious = sanityExperience.filter((e: any) => e.type === "Previous");

  const current = sanityCurrent.length > 0 ? sanityCurrent : hardcodedCurrent;
  const previous = sanityPrevious.length > 0 ? sanityPrevious : hardcodedPrevious;
            ...
            <div className="page-hero-grid">
              ...
              <div className="animate-fade-in delay-3 page-hero-photo">
                ...
                <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", borderRadius: "2px" }}>
                  <Image src="https://res.cloudinary.com/dmyrmlj5z/image/upload/q_auto/f_auto/v1776127496/568690701_18389482405130220_4097415862773396182_n_fpagkr.jpg" alt="Andy at a summit" fill style={{ objectFit: "cover", objectPosition: "center top" }} priority />
                </div>
      ...
      <section style={{ background: "#F5F2EA", padding: "6rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 5%" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "1.5rem" }}>
            {current.map((role: any, i: number) => (
              <Reveal key={i} delay={i * 0.1}>
                <div style={{ background: "#ffffff", borderRadius: "16px", padding: "2rem", border: "1px solid rgba(0,0,0,0.08)", borderTop: "3px solid #C9912A", height: "100%", transition: "all 0.3s" }}
      ...
      <section style={{ background: "#EDE8DC", padding: "6rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 5%" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.2rem" }}>
            {previous.map((role: any, i: number) => (
              <Reveal key={i} delay={i * 0.08}>
                ...
                >
                  ...
                  <div style={{ color: "#4a6070", fontSize: "0.78rem", marginBottom: "1rem" }}>{role.period}{role.location ? ` · ${role.location}` : ""}</div>
                  <p style={{ color: "#4a6070", fontSize: "0.84rem", lineHeight: 1.7, marginBottom: "1rem" }}>{role.description}</p>
                  {role.tags && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                      {role.tags.map((tag: string) => (
                        <span key={tag} style={{ fontSize: "0.68rem", padding: "0.2rem 0.65rem", borderRadius: "2rem", border: "1px solid rgba(0,0,0,0.1)", background: "#EDE8DC", color: "#2d3f4e", fontWeight: 500 }}>{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
