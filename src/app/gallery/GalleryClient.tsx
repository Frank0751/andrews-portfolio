...
export default function GalleryClient({ galleryItems }: { galleryItems: any[] }) {
  ...
  const photos = galleryItems.length > 0
    ? galleryItems.map((item: any) => ({
        src: item.image ? urlFor(item.image).width(800).url() : "",
        category: item.category,
      })).filter((item: any) => item.src)
    : hardcodedPhotos;

  const filtered = active === "All" ? photos : active === "Videos" ? [] : photos.filter((p: any) => p.category === active);
  ...
  return (
    <main>
        ...
        <div style={{ position: "relative", zIndex: 10, width: "100%", padding: "8rem 0 4rem" }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 5%" }}>
            <div className="page-hero-grid">
              ...
              <div className="animate-fade-in delay-3 page-hero-matrix">
                {[
                  ...
                  "https://res.cloudinary.com/dmyrmlj5z/image/upload/q_auto/f_auto/v1776125121/491498564_18366378268130220_2175622565025465249_n_xpjqfi.jpg",
                  "https://res.cloudinary.com/dmyrmlj5z/image/upload/q_auto/f_auto/v1776127852/656160964_18357772666231283_2151060228362153169_n_xjbuaa.jpg",
                ].map((src, i) => (
                  <div key={i} style={{ position: "relative", borderRadius: "8px", overflow: "hidden", gridRow: i === 0 ? "1 / 3" : "auto" }}>
                    <Image src={src} alt="Andy in action" fill style={{ objectFit: "cover", objectPosition: "center top" }} priority={i === 0} />
                  </div>
                ))}
              </div>
            </div>
          ...
          <Reveal style={{ display: "flex", gap: "0.75rem", marginBottom: "3rem", flexWrap: "wrap" }}>
            {categories.map((cat) => (
              <button key={cat} onClick={() => { setActive(cat); setLightbox(null); }}
                style={{ padding: "0.55rem 1.4rem", borderRadius: "2rem", border: active === cat ? "none" : "1px solid rgba(0,0,0,0.12)", background: active === cat ? "#C9912A" : "#ffffff", color: active === cat ? "#0a1628" : "#2d3f4e", fontWeight: 600, fontSize: "0.82rem", cursor: "pointer", transition: "all 0.2s", letterSpacing: "0.04em" }}>
                {cat}
              </button>
            ))}
          </Reveal>
                ...
                {videos.map((video, i) => (
                  <Reveal key={i} delay={i * 0.1}>
                    <div style={{ borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(0,0,0,0.08)", background: "#ffffff", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
                      <video controls poster={video.poster} style={{ width: "100%", display: "block", maxHeight: "400px", background: "#0a1628" }} preload="metadata">
                      ...
                      <div style={{ padding: "1.5rem 1.8rem" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
                          <div style={{ width: "2rem", height: "2rem", borderRadius: "50%", background: "rgba(201,145,42,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            <svg width="10" height="12" viewBox="0 0 10 12" fill="#C9912A"><path d="M0 0L10 6L0 12V0Z" /></svg>
                          </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1rem" }}>
                {filtered.map((photo: any, i: number) => (
                  <Reveal key={`${active}-${i}`} delay={i * 0.04}>
                    <div onClick={() => setLightbox(i)} style={{ position: "relative", height: i % 4 === 0 ? "320px" : "240px", borderRadius: "12px", overflow: "hidden", cursor: "pointer" }}>
                      <Image src={photo.src} alt="Andy" fill style={{ objectFit: "cover", transition: "transform 0.4s" }}
                        onMouseEnter={e => { (e.currentTarget as HTMLImageElement).style.transform = "scale(1.05)"; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLImageElement).style.transform = "scale(1)"; }}
                      />
                      <div style={{ position: "absolute", inset: 0, background: "rgba(201,145,42,0.08)", opacity: 0, transition: "opacity 0.3s" }}
      ...
      {lightbox !== null && filtered.length > 0 && (
        <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(5,12,22,0.97)", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); setLightbox((lightbox - 1 + filtered.length) % filtered.length); }} style={{ position: "absolute", left: "1.5rem", background: "rgba(255,255,255,0.1)", border: "none", borderRadius: "50%", width: "3rem", height: "3rem", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#ffffff", fontSize: "1.4rem" }}>‹</button>
          ...
          <button onClick={(e) => { e.stopPropagation(); setLightbox((lightbox + 1) % filtered.length); }} style={{ position: "absolute", right: "1.5rem", background: "rgba(255,255,255,0.1)", border: "none", borderRadius: "50%", width: "3rem", height: "3rem", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#ffffff", fontSize: "1.4rem" }}>›</button>
        </div>
      )}
